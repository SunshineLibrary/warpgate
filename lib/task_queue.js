...
(function() {
  ...
  TaskQueue.prototype.handleMessage = function(message, header, deliveryInfo, m) {
    var action, app, id, task;
    app = deliveryInfo.appId;
    id = deliveryInfo.messageId;
    action = deliveryInfo.type;
    if (app === 'warpgate' && id && action) {
      task = new Task(id, action, message, function(isSuccess) {
        if (isSuccess == null) {
          isSuccess = true;
        }
        if (isSuccess) {
